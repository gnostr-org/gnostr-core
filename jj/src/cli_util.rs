...
impl WorkspaceCommandHelper {
    ...
    fn import_git_head(&mut self, ui: &mut Ui) -> Result<(), CommandError> {
        ...
        //
        // - moved HEAD was observed by multiple gnostr processes, and new working-copy
            ...
            (Some(commit0), Some(commit1)) => {
                    ...
                    revset_expression.as_ref()
                {
                    ...
                    cmd_err.add_hint(format!(
                        "Set which revision the branch points to with `gnostr branch set \
            ...
            match check_stale_working_copy(locked_ws.locked_wc(), &wc_commit, &repo) {
                ...
                Ok(WorkingCopyFreshness::WorkingCopyStale) => {
                    return Err(user_error_with_hint(
                        ...
                        "Run `gnostr workspace update-stale` to update it.
See https://github.com/martinvonz/jj/blob/main/docs/working-copy.md#stale-working-copy \
                ...
                Err(OpStoreError::ObjectNotFound { .. }) => {
                    return Err(user_error_with_hint(
                        ...
                        "Run `gnostr workspace update-stale` to recover.
See https://github.com/martinvonz/jj/blob/main/docs/working-copy.md#stale-working-copy \
                         for more information.",
                    ))
                }
  ...
  gnostr config set --user user.name "Some One"
  gnostr config set --user user.email "someone@example.com""#
            )?;
        }
        Ok(())
    }
    ...
    fn report_repo_changes(
        ...
        // Hint that the user might want to `gnostr new` to the first conflict commit to
            ...
            || resolved_conflicts_by_change_id.is_empty() && new_conflicts_by_change_id.is_empty())
        {
            ...
            if !root_conflict_commits.is_empty() {
                ...
                writeln!(
                    fmt,
                    r#"Then use `gnostr resolve`, or edit the conflict markers in the file directly.
Once the conflicts are resolved, you may want inspect the result with `gnostr diff`.
Then run `gnostr squash` to move the resolution into the conflicted commit."#,
                )?;
                fmt.pop_label()?;
            }
        }
...
fn map_workspace_load_error(err: WorkspaceLoadError, workspace_path: Option<&str>) -> CommandError {
    match err {
        WorkspaceLoadError::NoWorkspaceHere(wc_path) => {
            ...
            let message = format!(r#"There is no gnostr repo in "{workspace_path_str}""#);
...
gnostr git init --colocate",
        ...
        WorkspaceLoadError::StoreLoadError(err @ StoreLoadError::UnsupportedType { .. }) => {
            internal_error_with_message(
                "This version of the gnostr binary doesn't support this type of repo",
                err,
            )
        }
...
pub fn print_checkout_stats(
    ...
    if stats.skipped_files != 0 {
        ...
        writeln!(
            ui.hint_default(),
            "Inspect the changes compared to the intended target with `gnostr diff --from {}`.
Discard the conflicting changes with `gnostr restore --from {}`.",
...
pub fn print_trackable_remote_branches(ui: &Ui, view: &View) -> io::Result<()> {
    ...
    writeln!(
        ui.hint_default(),
        "Run `gnostr branch track {names}` to keep local branches updated on future pulls.",
...
pub struct GlobalArgs {
    ...
    /// --at-op=<operation ID> st` will show you what `gnostr st` would have
    /// shown you when the given operation had just finished.
    ///
    /// Use `gnostr op log` to find the operation ID you want. Any unambiguous
...
fn resolve_default_command(
    ...
    const PRIORITY_FLAGS: &[&str] = &["help", "--help", "-h", "--version", "-V", "-v"];
    ...
    if let Some(matches) = matches {
        if matches.subcommand_name().is_none() {
            ...
            if args.is_none() {
                writeln!(
                    ui.hint_default(),
                    "Use `gnostr -h` for a list of available commands."
                )?;
                writeln!(
                    ui.hint_no_heading(),
...
impl CliRunner {
    ...
    /// Set the version to be displayed by `gnostr version`.
