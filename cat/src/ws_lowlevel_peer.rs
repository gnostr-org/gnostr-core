...
impl<T: Specifier> Specifier for WsLlClient<T> {
    fn construct(&self, p: ConstructParams) -> PeerConstructor {
        ...
        inner.map(move |q, _| get_ws_lowlevel_peer(WsLlContext::Client, q, opts.clone()))
    }
...
impl<T: Specifier> Specifier for WsLlServer<T> {
    fn construct(&self, p: ConstructParams) -> PeerConstructor {
        let inner = self.0.construct(p.clone());
        let opts = p.program_options;
        inner.map(move |q, _| get_ws_lowlevel_peer(WsLlContext::Server, q, opts.clone()))
    }
    specifier_boilerplate!(noglobalstate singleconnect has_subspec);
}
specifier_class!(
...
pub fn get_ws_lowlevel_peer(
    ...
    let c = websocket_base::codec::ws::MessageCodec::new_with_limits(
        mode,
        opts.max_ws_frame_length,
        opts.max_ws_message_length,
    );
    let hup = inner.2;
    inner.2 = None;
